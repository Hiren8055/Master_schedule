...
#########################################################################################
    ...
    #-------------------------FOR LABELS
        ...
        def intersection(station_dict):
            ...
            def intercept_selection_pts(y_target):
                print(station_dict["UP"][0],station_dict["UP"][1])
                # have to select the index of 10 or not then go for below 10 
                # find 10 in list and get the index 
                print(station_dict["UP"][0].index(y_target))
                intercept_index = station_dict["UP"][0].index(10)
                print(station_dict["UP"][1][intercept_index])
            ...
            x,y = intercept_selection_pts(y_target)
            x_target, y_target =  intercept(y_target, x ,y)
            axes[1][1].plot(x_target, y_target, 'o-')
